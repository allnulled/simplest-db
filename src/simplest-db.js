(function(factory, scope) {
    ...
    if(typeof window === "object") {
        window["SimplestDB"] = output;
    }
    if(typeof global === "object") {
        global["SimplestDB"] = output;
    }
    ...
    class SimplestDB {
        ...
        validateTable(tableId) {
            if(typeof tableId !== "string") throw new Error("Required «table=" + tableId + "» to be a string [0101]");
            if(!(tableId in this.schema.tables)) throw new Error("Required «table=" + tableId + "» to exist as table in schema, only accepted: «" + Object.keys(this.schema.tables).join("», «") + "» [0402]");
            return this.schema.tables[tableId];
        }
        
        validateRow(tableId, value) {
            if(typeof tableId !== "string") throw new Error("Required «table=" + tableId + "» to be a string [0801]");
            if(!(tableId in this.schema.tables)) throw new Error("Required «table=" + tableId + "» to exist as table in schema, only accepted: «" + Object.keys(this.schema.tables).join("», «") + "» [0802]");
            return true;
        }
        ...
        validateSchema(schema) {
            ...
            if(typeof schema.attributes !== "object") throw new Error("Required «schema.attributes» to be an object [0303]");
            if(typeof schema.tables === "undefined") schema.tables = {};
            if(typeof schema.tables !== "object") throw new Error("Required «schema.tables» to be an object [0304]");
            ...
            for(let indexTable = 0; indexTable < tableIds.length; indexTable++) {
                ...
                if(typeof schema.tables[tableId] !== "object") throw new Error("Required «schema.tables[" + JSON.stringify(tableId) + "]» to be an object [0305]");
                if(typeof schema.tables[tableId].attributes === "undefined") schema.tables[tableId].attributes = {};
                if(typeof schema.tables[tableId].attributes !== "object") throw new Error("Required «schema.tables[" + JSON.stringify(tableId) + "].attributes» to be an object [0306]");
                if(typeof schema.tables[tableId].columns === "undefined") schema.tables[tableId].columns = {};
                if(typeof schema.tables[tableId].columns !== "object") throw new Error("Required «schema.tables[" + JSON.stringify(tableId) + "].columns» to be an object [0307]");
        ...
        consumeIdOf(tableId) {
            if(typeof tableId !== "string") throw new Error("Required «tableId» to be an object [0901]");
        ...
        getData(tableId) {
            if(typeof tableId !== "string") throw new Error("Required «tableId» to be an object [0401]");
            this.validateTable(tableId);
            if(typeof window === "object") {
                const storageId = "SDB_STORAGE_FOR_" + this.schema.schema;
                if(!(storageId in localStorage)) {
        ...
        setData(tableId, modelId, data) {
            if(typeof window === "object") {
                if(typeof tableId !== "string") throw new Error("Required «tableId» to be an string [0501]");
                if(typeof modelId !== "number") throw new Error("Required «modelId» to be an number [0502]");
                if(typeof data === "undefined") {}
                else if(typeof data !== "object") throw new Error("Required «data» to be an object [0503]");
                this.validateTable(tableId);
                const storageId = "SDB_STORAGE_FOR_" + this.schema.schema;
                if(!(storageId in localStorage)) {
                    localStorage[storageId] = JSON.stringify({$KEYS:{}});
                ...
                if(!(selectedId in storageData[tableId])) {
                    if(modelId !== 0) {
                        throw new Error("Required «modelId=" + modelId + "» to be 0 or to exist as id in table «" + storageId + ":" + tableId + "» [0504]")
                ...
                if(typeof tableId !== "string") throw new Error("Required «tableId» to be an string [1101]");
                if(typeof modelId !== "number") throw new Error("Required «modelId» to be an number [1102]");
                if(typeof data === "undefined") {}
                else if(typeof data !== "object") throw new Error("Required «data» to be an object [1103]");
                this.validateTable(tableId);
                const fs = require("fs");
                const storageId = this.baseDir + this.schema.schema + ".data.json";
                if(!fs.existsSync(storageId)) {
                ...
                if(!(selectedId in storageData[tableId])) {
                    if(modelId !== 0) {
                        throw new Error("Required «modelId=" + modelId + "» to be 0 or to exist as id in table «" + storageId + ":" + tableId + "#" + modelId + "» [1104]")
                    } else operation = "insert";
                }
                if(typeof data === "undefined") {
                    delete storageData[tableId][selectedId];
        ...
        select(tableId, filter) {
            if(typeof tableId !== "string") throw new Error("Required «tableId» to be an string [0601]");
            this.validateTable(tableId);
            ...
            if(typeof filter === "function") {
                ...
                throw new Error("Required «filter» to be a valid type [0602]");
            }
        }

        insert(tableId, value) {
            if(typeof tableId !== "string") throw new Error("Required «tableId» to be an string [0701]");
            if(typeof value !== "object") throw new Error("Required «value» to be an object [0702]");
            this.validateRow(tableId, value);
            return this.setData(tableId, 0, value);
        }

        update(tableId, instanceId, value) {
            if(typeof tableId !== "string") throw new Error("Required «tableId» to be an string [1201]");
            if(typeof instanceId !== "number") throw new Error("Required «instanceId» to be an number [1202]");
            if(typeof value !== "object") throw new Error("Required «value» to be an object [1203]");
            this.validateRow(tableId, value);
            return this.setData(tableId, instanceId, value);
        }

        delete(tableId, instanceId) {
            if(typeof tableId !== "string") throw new Error("Required «tableId» to be an string [1401]");
            if(typeof instanceId !== "number") throw new Error("Required «instanceId» to be an number [1402]");
            this.validateTable(tableId);
